...
# # loading the Markdown document (assumes info.md is in DATA_PATH)
...
# # The persist() method is no longer needed in this version.
# # vector_store.persist()

# print(f"Successfully stored {len(chunks)} chunks from the Markdown file into the vector store.")
...
# Load the Markdown document (assumes info.md is in DATA_PATH)
markdown_file = f"{DATA_PATH}/info.md"
loader = UnstructuredMarkdownLoader(file_path=markdown_file)
raw_documents = loader.load()

# Since the document is small, we use it as a single document without chunking.
doc_ids = [str(uuid4()) for _ in range(len(raw_documents))]

# Add the document(s) to the vector store
